...
export class TicketResult extends Component {
  constructor(props) {
    super(props);
    this.state = {
      userId: props.location.state ? props.location.state.setDataBooking.userId : "",
      movieId: props.location.state ? props.location.state.setDataBooking.movieId : "",
      scheduleId: props.location.state ? props.location.state.setDataBooking.scheduleId : "",
      dateBooking: props.location.state ? props.location.state.setDataBooking.dateBooking : "",
      timeBooking: props.location.state ? props.location.state.setDataBooking.timeBooking : "",
      seat: props.location.state ? props.location.state.setDataBooking.seat : "",
      paymentMethod: props.location.state ? props.location.state.setDataBooking.paymentMethod : "",
      movieName: props.location.state ? props.location.state.setDataBooking.movieName : "",
      tickets: [],
      isLoading: false
    };
  ...
  getUserBuyTicket = async () => {
    try {
      const response = await axios.get(`booking/export-ticket/${this.state.userId}`);
      this.setState({
        isLoading: true,
        ...
        <main className="ticket__result-main">
          <section className="ticket__result-container">
            {this.state.isLoading ? (
              <section className="ticket__result-card">
                ...
                <div className="ticket__result-body">
                  ...
                  <div className="ticket__result-body-total d-flex d-md-none">
                    <span>Total</span>
                    <span className="fw-bold">${this.state.tickets.totalPayment}</span>
                  </div>
                </div>
              ...
              <p className="text-center fs-4 fw-bold">Memuat hasil tiket, silahkan tunggu...</p>
            )}
          </section>
        </main>
        <Footer />
